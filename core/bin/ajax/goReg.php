...
$sql = $db->query("SELECT * FROM REGISTRO WHERE Nombre_Usuario='$user' OR Email='$email' LIMIT 1;");
if($db->rows($sql) == 0) {
  $keyreg = md5(time());
  ...
  if(!$mail->send()) {
    ...
    $sql_2 = $db->query("SELECT MAX(Id_Usuario) AS id FROM REGISTRO;");
    $id = $db->recorrer($sql_2)[0] + 1;
    $db->query("INSERT INTO REGISTRO (Id_Usuario,Nombre_Usuario,Email,pass,keyreg) VALUES ($id,'$user','$email','$pass','$keyreg');");
    $_SESSION['app_id'] = $id;
